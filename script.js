...
function clearAllData(){if(!confirm('This will delete ALL your data (history, settings, achievements). Continue?'))return;try{Object.keys(localStorage).forEach(k=>{if(k.startsWith('bwatery_'))localStorage.removeItem(k)})}catch(e){}state.gender=null;state.age=null;state.weight=null;state.dailyGoal=3000;state.currentIntake=0;state.darkMode=false;state.reminderInterval=30;state.weatherBoost=true;state.lastDate=null;state._weatherBoosted=false;state.onboarded=false;state.userName='';state.soundEnabled=true;state.wakeTime='07:00';state.bedTime='23:00';state.reminderMode='sound';state.remindAfterGoal=false;state.unit='ml';state.defaultCup=250;state.language='en';saveState();applyDarkMode();showScreen('screen-onboarding')}
...
function startReminderTimer(){if(state.reminderTimer)clearInterval(state.reminderTimer);if(state.reminderMode==='off')return;requestNotifyPermission();state.reminderTimer=setInterval(()=>{const pct=state.currentIntake/state.dailyGoal;const now=new Date();const h=now.getHours(),m=now.getMinutes();const currentTime=h*60+m;const [wH,wM]=(state.wakeTime||'07:00').split(':').map(Number);const [bH,bM]=(state.bedTime||'23:00').split(':').map(Number);const wakeMin=wH*60+wM;const bedMin=bH*60+bM;const isAwake=bedMin>wakeMin?(currentTime>=wakeMin&&currentTime<=bedMin):(currentTime>=wakeMin||currentTime<=bedMin);const exp=Math.min(.85,(currentTime/(24*60))*1.2);if(isAwake&&(pct<exp||state.remindAfterGoal)){if(state.reminderMode==='sound'||state.reminderMode==='soundOnly')playWaterDrop();if(state.reminderMode==='vibrate'&&navigator.vibrate)navigator.vibrate(200);if(state.reminderMode==='sound'&&navigator.vibrate)navigator.vibrate(200);const p=document.getElementById('reminderPopup');if(p&&!p.classList.contains('active'))p.classList.add('active');if(state.reminderMode!=='display'&&state.reminderMode!=='off'&&'Notification' in window&&Notification.permission==='granted')new Notification('💧 Time to Drink!',{body:'You\'re behind on your goal. ('+fmtL(state.currentIntake)+' / '+fmtL(state.dailyGoal)+')'})}},state.reminderInterval*60*1000)}
...
function updateReminderMode(val){state.reminderMode=val;saveState();if(state.reminderTimer){clearInterval(state.reminderTimer);if(state.reminderInterval>0)startReminderTimer()}saveUserData()}
...
function renderCommentWithReplies(s,depth){depth=depth||0;const indent=depth>0?'margin-left:20px;padding-left:12px;border-left:2px solid var(--border)':'';const ratingHTML=s.rating?'<div style="margin-top:4px">'+renderStarsHTML(s.rating)+'</div>':'';const nameColor=escHtml(s.name)===(state.userName||'')?'color:var(--primary)':'';let repliesHTML='';if(s.replies&&s.replies.length){repliesHTML='<div class="comment-replies">';s.replies.forEach(r=>{const rn=r.name===(state.userName||'')?'color:var(--primary)':'';repliesHTML+='<div class="comment-reply"><span class="comment-name" style="'+rn+'">'+escHtml(r.name)+'</span><span class="comment-text">'+formatMentions(escHtml(r.text))+'</span><span class="comment-date">'+escHtml(r.date||'')+'</span></div>'});repliesHTML+='</div>'}return'<div class="user-comment-item" style="'+indent+'" data-id="'+escHtml(s.id||'')+'"><div class="comment-header"><span class="comment-name" style="'+nameColor+'">'+escHtml(s.name)+'</span><span class="comment-date">'+escHtml(s.date||'')+'</span></div><span class="comment-text">'+formatMentions(escHtml(s.text))+'</span>'+ratingHTML+'<div class="comment-actions"><button class="comment-reply-btn" onclick="toggleReplyForm(\''+escHtml(s.id||'')+'\')">↩ Reply</button></div><div class="reply-form" id="replyForm-'+escHtml(s.id||'')+'" style="display:none"><input type="text" class="reply-input" id="replyInput-'+escHtml(s.id||'')+'" placeholder="Write a reply... Use @ to mention" oninput="handleMentionInput(event,\''+escHtml(s.id||'')+'\')" onkeydown="handleReplyKey(event,\''+escHtml(s.id||'')+'\')"><div class="mention-dropdown" id="mentionDrop-'+escHtml(s.id||'')+'" style="display:none"></div><button class="reply-submit-btn" onclick="submitReply(\''+escHtml(s.id||'')+'\')">Send</button></div>'+repliesHTML+'</div>'}

function formatMentions(text){return text.replace(/@(\w+)/g,'<span style="color:var(--primary);font-weight:600">@$1</span>')}
...
function getRecentNames(){if(!db)return Promise.resolve([]);return db.collection('feedback').orderBy('timestamp','desc').limit(100).get().then(snap=>{const names=new Set();snap.forEach(doc=>{const d=doc.data();if(d.name)names.add(d.name);if(d.replies)d.replies.forEach(r=>{if(r.name)names.add(r.name)})});return[...names]}).catch(()=>[])}

function toggleReplyForm(fid){const form=document.getElementById('replyForm-'+fid);if(!form)return;const isHidden=form.style.display==='none';document.querySelectorAll('.reply-form').forEach(f=>f.style.display='none');if(isHidden)form.style.display='flex'}

function submitReply(fid){const input=document.getElementById('replyInput-'+fid);if(!input)return;const text=input.value.trim();if(!text)return;const name=state.userName||'Anonymous';const reply={name:name,text:text,date:getToday(),timestamp:Date.now()};if(db){db.collection('feedback').doc(fid).update({replies:firebase.firestore.FieldValue.arrayUnion(reply)}).then(()=>{input.value='';document.getElementById('replyForm-'+fid).style.display='none';renderUserComments()}).catch(e=>console.error('Reply failed:',e))}else{let s=[];try{s=JSON.parse(localStorage.getItem('bwatery_suggestions')||'[]')}catch(e){}const item=s.find(x=>x.timestamp===parseInt(fid));if(item){if(!item.replies)item.replies=[];item.replies.push(reply);localStorage.setItem('bwatery_suggestions',JSON.stringify(s));input.value='';document.getElementById('replyForm-'+fid).style.display='none';renderUserComments()}}}

function toggleExpandComments(){commentsExpanded=!commentsExpanded;renderUserComments()}
function renderUserComments(){const inner=document.getElementById('userCommentsInner');const manage=document.getElementById('feedbackManageList');const btn=document.getElementById('expandCommentsBtn');if(!inner)return;if(db){db.collection('feedback').orderBy('timestamp','desc').limit(50).onSnapshot(snap=>{const items=[];snap.forEach(doc=>items.push({id:doc.id,...doc.data()}));if(!items.length){inner.innerHTML='<div class="user-comments-empty">No experiences yet. Be the first to share!</div>';if(manage)manage.innerHTML='';if(btn)btn.style.display='none';return}const shown=commentsExpanded?items:items.slice(0,2);inner.innerHTML='';shown.forEach(s=>{inner.innerHTML+=renderCommentWithReplies(s)});if(btn){if(items.length>2){btn.textContent=commentsExpanded?'Show less':'Show all '+items.length+' experiences';btn.style.display='block'}else{btn.style.display='none'}}if(manage)renderManageList(items)},()=>{renderUserCommentsLocal(inner);if(manage)renderManageListLocal(manage)})}else{renderUserCommentsLocal(inner);if(manage)renderManageListLocal(manage)}}
function renderUserCommentsLocal(inner){let suggestions=[];try{suggestions=JSON.parse(localStorage.getItem('bwatery_suggestions')||'[]')}catch(e){}const btn=document.getElementById('expandCommentsBtn');if(!suggestions.length){inner.innerHTML='<div class="user-comments-empty">No experiences yet. Be the first to share!</div>';if(btn)btn.style.display='none';return}const shown=commentsExpanded?suggestions:suggestions.slice(0,2);inner.innerHTML='';shown.forEach(s=>{inner.innerHTML+=renderCommentWithReplies(s)});if(btn){if(suggestions.length>2){btn.textContent=commentsExpanded?'Show less':'Show all '+suggestions.length+' experiences';btn.style.display='block'}else{btn.style.display='none'}}}
function renderManageList(items){const el=document.getElementById('feedbackManageList');if(!el)return;if(!items.length){el.innerHTML='';return}el.innerHTML='';items.forEach(s=>{const card=document.createElement('div');card.style.cssText='background:var(--surface-1);border:1px solid var(--border);border-radius:8px;padding:10px 12px;display:flex;flex-direction:column;gap:6px';card.innerHTML='<div style="font-size:12px;color:var(--text-secondary)"><b style="color:var(--text)">'+escHtml(s.name)+'</b> · '+escHtml(s.date)+' '+(s.rating?renderStarsHTML(s.rating):'')+'</div><div style="font-size:13px;color:var(--text)">'+escHtml(s.text)+'</div><div style="display:flex;gap:6px;justify-content:flex-end"><button onclick="editFeedback(\''+s.id+'\',\''+escHtml(s.name).replace(/'/g,"\\'")+'\',\''+escHtml(s.text).replace(/'/g,"\\'")+'\')" style="padding:3px 8px;border-radius:6px;border:1px solid var(--primary);background:transparent;color:var(--primary);font-size:11px;cursor:pointer;font-family:inherit">Edit</button><button onclick="deleteFeedback(\''+s.id+'\')" style="padding:3px 8px;border-radius:6px;border:1px solid var(--danger);background:transparent;color:var(--danger);font-size:11px;cursor:pointer;font-family:inherit">Delete</button></div>';el.appendChild(card)})}
function renderManageListLocal(el){let suggestions=[];try{suggestions=JSON.parse(localStorage.getItem('bwatery_suggestions')||'[]')}catch(e){}if(!suggestions.length){el.innerHTML='';return}el.innerHTML='';suggestions.forEach((s,i)=>{const card=document.createElement('div');card.style.cssText='background:var(--surface-1);border:1px solid var(--border);border-radius:8px;padding:10px 12px;display:flex;flex-direction:column;gap:6px';card.innerHTML='<div style="font-size:12px;color:var(--text-secondary)"><b style="color:var(--text)">'+escHtml(s.name)+'</b> · '+escHtml(s.date)+' '+(s.rating?renderStarsHTML(s.rating):'')+'</div><div style="font-size:13px;color:var(--text)">'+escHtml(s.text)+'</div><div style="display:flex;gap:6px;justify-content:flex-end"><button onclick="editFeedbackLocal('+i+')" style="padding:3px 8px;border-radius:6px;border:1px solid var(--primary);background:transparent;color:var(--primary);font-size:11px;cursor:pointer;font-family:inherit">Edit</button><button onclick="deleteFeedbackLocal('+i+')" style="padding:3px 8px;border-radius:6px;border:1px solid var(--danger);background:transparent;color:var(--danger);font-size:11px;cursor:pointer;font-family:inherit">Delete</button></div>';el.appendChild(card)})}

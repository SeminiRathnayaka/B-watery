...
function renderUserComments(){const inner=document.getElementById('userCommentsInner');const manage=document.getElementById('feedbackManageList');if(!inner)return;if(db){db.collection('feedback').orderBy('timestamp','desc').limit(50).onSnapshot(snap=>{const items=[];snap.forEach(doc=>items.push({id:doc.id,...doc.data()}));if(!items.length){inner.innerHTML='<div class="user-comments-empty">No experiences yet. Be the first to share!</div>';if(manage)manage.innerHTML='';return}inner.innerHTML='';items.forEach(s=>{inner.innerHTML+=renderCommentWithReplies(s)});if(manage)renderManageList(items)},()=>{renderUserCommentsLocal(inner);if(manage)renderManageListLocal(manage)})}else{renderUserCommentsLocal(inner);if(manage)renderManageListLocal(manage)}}
function renderUserCommentsLocal(inner){let suggestions=[];try{suggestions=JSON.parse(localStorage.getItem('bwatery_suggestions')||'[]')}catch(e){}if(!suggestions.length){inner.innerHTML='<div class="user-comments-empty">No experiences yet. Be the first to share!</div>';return}inner.innerHTML='';suggestions.forEach(s=>{inner.innerHTML+=renderCommentWithReplies(s)})}
